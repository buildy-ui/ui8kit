...
export function useScrollToBottom<T extends HTMLElement>(): [
  ...
  const getViewport = useCallback((element: HTMLElement | null) => {
    // ui8kit-neutral viewport: any ancestor with data-class="scroll-viewport"
    return element?.closest('[data-class="scroll-viewport"]') as HTMLElement;
  }, []);
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      if (!isUserScrolling.current) updateScrollState(viewport);
    };
